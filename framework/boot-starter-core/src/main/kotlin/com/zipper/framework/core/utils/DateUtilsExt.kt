...
object DateUtilsExt : DateUtils() {
    ...
    fun dateTime(format: String, ts: String?): Date {
        try {
            return SimpleDateFormat(format).parse(ts)
    ...
    fun parseDate(str: Any?): Date? {
        ...
        return try {
            parseDate(str.toString(), *PARSE_PATTERNS)
        } catch (e: ParseException) {
            null
        }
    }
    ...
    fun differentDaysByMillisecond(date1: Date, date2: Date): Int {
        return abs(((date2.time - date1.time) / (1000 * 3600 * 24)).toInt().toDouble())
            .toInt()
    }
