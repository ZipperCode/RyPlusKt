...
import com.zipper.framework.email.utils.Mail.Companion.create
...
object MailUtils {
     ...
     * 获取邮件发送实例
     */
    private val mailAccount: MailAccount by lazy {
        SpringUtil.getBean(MailAccount::class.java)
    }
    ...
    fun getMailAccount(from: String, user: String, pass: String): MailAccount {
        mailAccount.setFrom(from.ifBlank { mailAccount.from })
        mailAccount.setUser(user.ifBlank { mailAccount.user })
        mailAccount.setPass(pass.ifBlank { mailAccount.pass })
        return mailAccount
    }
     ...
     * @since 4.0.3
     */
    fun send(to: String?, cc: String?, bcc: String?, subject: String?, content: String?, isHtml: Boolean, vararg files: File): String {
    ...
    fun getSession(mailAccount: MailAccount, isSingleton: Boolean): Session {
        ...
        return if (isSingleton) Session.getDefaultInstance(mailAccount.smtpProps, authenticator) //
        else Session.getInstance(mailAccount.smtpProps, authenticator)
    }
    ...
    private fun splitAddress(addresses: String?): List<String?> {
        ...
        val result = if (StrUtil.contains(addresses, CharUtil.COMMA)) {
