package com.zipper.framework.log.aspect
...
import com.zipper.framework.log.enums.BusinessStatus
...
class LogAspect {
    ...
    protected fun handleLog(joinPoint: JoinPoint, controllerLog: Log, e: Exception?, jsonResult: Any?) {
        try {
            ...
            // 发布事件保存数据库
            SpringUtilExt.context().publishEvent(operLog)
        } catch (exp: Exception) {
            // 记录本地异常日志
    ...
    companion object {
         ...
         * 计算操作消耗时间
         */
        private val TIME_THREADLOCAL: ThreadLocal<StopWatch> = TransmittableThreadLocal()
    }
}
